...
namespace DekorStone_API.Models.Db
{
    public class DbInsert
    {
        ...
        public ResponseStruct<int> InsertOrder(string userToken,string requestToken,
            string clientFullName,long clientPhone, double kvm, int modelID,double price,double payment,string note,string endDate){
            ...
            try
            {
                ...
                if (userID1 == userID2 && userID1 > 0 && userID2 > 0)
                {
                    ...
                    using (MySqlConnection connection = new MySqlConnection(ConnectionString))
                    {
                        ...
                        long todaySumID = select.GetTodaySumID();
                        if (todaySumID > 0)
                        {
                            using (MySqlCommand com = new MySqlCommand(@"update daily_sum set dayEndMoney=dayEndMoney+@dayEndMoney, income=income+@income where id=@todaySumID", connection))

                            {
                                com.Parameters.AddWithValue("@dayEndMoney", payment);
                                com.Parameters.AddWithValue("@income", payment);
                                com.Parameters.AddWithValue("@todaySumID", todaySumID);
                        ...
                        {
                            DailySumStruct sum = select.GetYesterdaySum();
                            using (MySqlCommand com = new MySqlCommand(@"INSERT INTO daily_sum (dayStartMoney,dayEndMoney,income,outcome,cdate)
                      Values (@dayStartMoney,@dayEndMoney,@income,@outcome,@cdate)", connection))

                            {
                                com.Parameters.AddWithValue("@dayStartMoney", sum.dayEndMoney);
                                com.Parameters.AddWithValue("@dayEndMoney", (sum.dayEndMoney + payment) - sum.outcome);
                                com.Parameters.AddWithValue("@income", payment);
                                com.Parameters.AddWithValue("@outcome", 0);
                                com.Parameters.AddWithValue("@cdate", now);

                                com.ExecuteNonQuery();
                                todaySumID = com.LastInsertedId;
                            }

                        }
                        
                        
                        using (MySqlCommand com = new MySqlCommand(@"INSERT INTO orders (userID,clientFullname,clientPhone,kv_m,modelID,price,payment,note,status,cdate,enddate,dailySumID)
                      Values (@userID,@fullName,@clientPhone,@kv_m,@modelID,@price,@payment,@note, @status, @cdate,@endDate,@dailySumID)", connection))

                        {
                            com.Parameters.AddWithValue("@userID", userID1);
                            com.Parameters.AddWithValue("@fullName", clientFullName);
                            com.Parameters.AddWithValue("@clientPhone", clientPhone);

                            com.Parameters.AddWithValue("@kv_m", kvm);
                            ...
                            com.Parameters.AddWithValue("@cdate", now);
                            com.Parameters.AddWithValue("@endDate",DateTime.Parse(endDate));
                            com.Parameters.AddWithValue("@dailySumID", todaySumID);
                            com.ExecuteNonQuery();
                        ...
                        {
                            response.status = 2; // not inserted (error)
                        }
                    }
